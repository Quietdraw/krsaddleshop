				...
				<li>
					<a	href="{{link}}"
						...
						{{#if isImageTile}}
							...
							<div class="product-color-swatch-name">{{label}}</div>
						{{/if}}
						{{#if isColorTile}}
						<span class="product-color-swatch" style="background: {{color}}"></span> <div class="product-color-swatch-name">{{label}}</div>
						{{/if}}
					</a>
				</li>
			{{/if}}
